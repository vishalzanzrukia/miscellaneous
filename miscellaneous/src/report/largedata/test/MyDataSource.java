...
public class MyDataSource implements JRDataSource {
	...
	public boolean next() throws JRException {
		...
		if (getTotalNumOfRecords() == -1) {
			/** needs to load total number of records. This will execute only once for one report */
			setTotalNumOfRecords(DataProvider.getTotalNumOfRecords());
		}

		if (globalCounter < DataProvider.getTotalNumOfRecords()) {
			globalCounter++;
			isNext = true;
		}
